...
class ComicDetailPage extends StatelessWidget {
  ...
  const ComicDetailPage({Key key, this.id, this.title, this.model})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    // TODO: implement build
    return ScreenAdaptWidget(
        platform: TEAdaptPlatform.padLandscape.toString(),
        autoOrientation: true,
        child: ComicDetailFlexPage(
          id: id,
          title: title,
          model: model,
        ));
...
class ComicDetailFlexPage extends StatefulWidget {
  final String id;
  final String title;
  final BaseSourceModel model;

  ComicDetailFlexPage({this.id, this.title, this.model});
...
class _ComicDetailPage extends FlexibleState<ComicDetailFlexPage> {
  bool _lock = true;
...
//     // TODO: implement build
    ...
    return Scaffold(
                      ...
                      if (Provider.of<ComicDetailModel>(context, listen: false)
                        ...
                        Navigator.push(
                            ...
                            MaterialPageRoute(
                                ...
                                settings:
                                    RouteSettings(name: 'comic_view_page')));
                      } else if (Provider.of<ComicDetailModel>(context,
                                  listen: false)
                              .lastChapterId ==
                          '') {
        ...
        body: DirectSelectContainer(
          child: EasyRefresh(
            ...
            emptyWidget: Provider.of<ComicDetailModel>(context).error != null
                  ...
                  children: <Widget>[
                    Expanded(
                      child: Parallax.inside(
                        child: Image(
                            image: buildProvider(context), fit: BoxFit.cover),
                        ...
                        children: [
                          ...
                          TextButton.icon(
                              onPressed: () {
                                Provider.of<ComicDetailModel>(context,
                                        listen: false)
                                    .reverse = !Provider.of<ComicDetailModel>(
                                        context,
                                        listen: false)
                                    .reverse;
                              },
                              icon: Icon(
                                  Provider.of<ComicDetailModel>(context).reverse
  ...
  Widget buildPhone(BuildContext context) {
    ...
    return ChangeNotifierProvider(
        create: (_) => ComicDetailModel(
            Provider.of<SourceProvider>(context).active,
            widget.title,
            widget.id),
  ...
  Widget buildPadDetail(BuildContext context){
    return Scaffold(
        appBar: AppBar(
          title: Text(Provider.of<ComicDetailModel>(context).title),
          actions: <Widget>[
          ...
          builder: (context) {
            return FancyFab(
              isSubscribe: Provider.of<TrackerModel>(context)
                  .ifSubscribe(Provider.of<ComicDetailModel>(context)),
              onMessage: () {
                _navigator.currentState.pushReplacement(MaterialPageRoute(builder: (context)=>CommentPage(detail: Provider.of<ComicDetailModel>(context).detail,)));
              },
              onPlay: () async {
                if (Provider.of<ComicDetailModel>(context, listen: false)
                    .lastChapterId !=
            ...
            child: new Column(
              children: <Widget>[
                ...
                Card(
                  ...
                  child: Column(
                    children: Provider.of<ComicDetailModel>(context)
                        .buildChapterWidgetList(context,navigatorKey: _navigator),
                  ),
                )
              ],
            ),
